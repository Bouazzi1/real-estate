...
export default async function LandingPage({ searchParams }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950 text-slate-800 dark:text-slate-100 flex flex-col justify-between selection:bg-blue-500 selection:text-white">
      ...
      <div className="flex-1">
        {sections.map((section) => {
          ...
          switch (section.key) {
            case "HERO":
              return (
                <section key={section.id} className="relative min-h-[85vh] flex items-center justify-center bg-slate-950 text-white px-6 overflow-hidden">
            ...
            case "STATS":
              return (
                <section key={section.id} className="py-12 bg-slate-100 dark:bg-slate-900 border-t border-b border-slate-200/50 dark:border-slate-800/50 relative z-10">
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="p-6 space-y-4">
                                ...
                                <div className="grid grid-cols-3 gap-2 border-t border-b border-slate-100 dark:border-slate-800/50 py-3 text-[10px] text-slate-500 dark:text-slate-400 text-center">
                                  <div>
                                    <span className="font-bold text-slate-800 dark:text-slate-200 block">{apt.surface} m²</span>
                                    <span className="text-[9px] text-slate-500 block mt-0.5">Surface</span>
                                  </div>
                                  <div>
                                    <span className="font-bold text-slate-800 dark:text-slate-200 block">{apt.bedrooms} Bed</span>
                                    <span className="text-[9px] text-slate-500 block mt-0.5">Rooms</span>
                                  </div>
                                  <div>
                                    <span className="font-bold text-slate-800 dark:text-slate-200 block">{apt.orientation}</span>
                                ...
                                <div className="flex gap-2">
                                  <Link
                                    ...
                                    className="flex-1 flex items-center justify-center gap-2 py-2 px-3 bg-slate-100 hover:bg-slate-200 dark:bg-slate-950 dark:hover:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-200 text-xs font-semibold rounded-xl transition-colors cursor-pointer"
                                  >
                                    <span>Details</span>
            ...
            case "ABOUT":
              return (
                <section key={section.id} id="about" className="py-24 bg-white dark:bg-slate-900/10 border-t border-slate-200/50 dark:border-slate-800/50">
                  <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                      ...
                      {/* Right: Cover Photo */}
                      {content.imageUrl && (
                        <div className="relative rounded-3xl overflow-hidden border border-slate-200 dark:border-slate-800 aspect-video lg:aspect-square shadow-2xl">
                  ...
                  <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      {content.items?.map((item: any, idx: number) => (
                        <div
                          key={idx}
                          className="p-8 bg-white dark:bg-slate-900/30 border border-slate-200/80 dark:border-slate-800/80 rounded-3xl backdrop-blur-xl flex flex-col justify-between gap-6"
                        >
      ...
      {/* Footer block */}
      <footer className="bg-slate-950 text-slate-400 border-t border-slate-900/50 py-16 px-6 relative z-10">
        ...
        <div className="max-w-7xl mx-auto border-t border-slate-900/60 mt-12 pt-8 text-center text-[10px] text-slate-600">
